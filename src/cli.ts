...
import { watchRouteArtifacts } from './watch.js'
...
export async function run(argv = process.argv.slice(2)): Promise<number> {
  ...
  try {
    if (options.watch) {
      ...
      let watcher = await watchRouteArtifacts(options, {
    ...
    let result = await writeRouteArtifacts(options)
    printResult(result)
...
const help = `Usage: remix-fs-routes [generate] [options]

Generate Remix 3 routes and a controller from route folders.
